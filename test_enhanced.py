#!/usr/bin/env python3
"""
Test script to test enhanced strategy with enhanced backtester
"""
import pandas as pd
...
def get_enhanced_config():
    """Configuration for enhanced strategy with aggressive filtering"""
    config = {
        "mode": "backtest",
        "strategy": {
            ...
            "position_size": 0.001,
            "max_positions": 1,
            # Enhanced parameters - more aggressive filtering
            "rsi_period": 14,
            "ema_period": 50,
            "atr_period": 14,
            "volume_period": 20,
            "rsi_oversold": 25,      # More restrictive RSI
            "rsi_overbought": 75,    # More restrictive RSI
            "volume_multiplier": 1.5, # Require higher volume
            "atr_multiplier_sl": 1.5, # Tighter stop loss
            "atr_multiplier_tp": 4.0, # Higher take profit for better R:R
            "imbalance_threshold": 0.0005  # Require larger gaps
        },
        "backtest": {
            "start": "2025-05-25",
            "end": "2025-08-01"
...
def main():
    """Test enhanced strategy with enhanced backtester"""
    logger = setup_logger()
    broker = CSVBroker("BTCUSDT_4h_ohlcv.csv", logger)
    
    print("="*80)
    print("ENHANCED STRATEGY with ENHANCED BACKTESTER")
    print("="*80)
    
    # Enhanced strategy with enhanced backtester
